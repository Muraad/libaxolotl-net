...
namespace Tests
{
	...
	public class RootKeyTest
	{
		...
		public void TestRootKeyDerivationV2 ()
		{
			...
			ECKeyPair    aliceKeyPair    = new ECKeyPair(alicePrivateKey, alicePublicKey);

			ECPublicKey bobPublicKey = Curve.DecodePoint(bobPublic, 0);
			RootKey     rootKey      = new RootKey(HKDF.CreateFor(2), rootKeySeed);

			Tuple<RootKey, ChainKey> rootKeyChainKeyPair = rootKey.CreateChain(bobPublicKey, aliceKeyPair);
			RootKey                 nextRootKey         = rootKeyChainKeyPair.Item1;
			ChainKey                nextChainKey        = rootKeyChainKeyPair.Item2;

			Assert.True(ArrayComparer.Compare(rootKey.Key, rootKeySeed));
			Assert.True(ArrayComparer.Compare(nextRootKey.Key, nextRoot));
			Assert.True(ArrayComparer.Compare(nextChainKey.Key, nextChain));
		}
	}
}
