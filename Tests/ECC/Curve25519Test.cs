	...
	public class Curve25519Test
	{
		...
		public void TestRandomAgreements()
		{
			for (int i = 0 ;i < 50; i++) {
				...
				byte[]    sharedAlice = Curve.CalculateAgreement(bob.PublicKey, alice.PrivateKey);
				byte[]    sharedBob   = Curve.CalculateAgreement(alice.PublicKey, bob.PrivateKey);

				Assert.True (ArrayComparer.Compare (sharedAlice, sharedBob));
			}
		}
	}
}
